...
| `task-108` | `ms-02` | Design Python runtime dependency detection and on-run installation flow | Edward + Jules | 5 | 24 | `done` | GitHub issue: #5. The application must detect missing libraries and bootstrap them dynamically when possible. Refined by Jules with structured `BootstrapPolicy` and `BootstrapResult` types. |
